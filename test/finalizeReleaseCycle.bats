...
@test "ignores prerelease values in STABLE_PACKAGE_VERSIONS when refreshing Published versions" {
  local work
  work="${BATS_TEST_TMPDIR}/skip-prerelease"
  mkdir -p "$work/.releases"
  cp -a "$PROJECT_ROOT/test/fixtures/release-cycles/2026.05.08.1" "$work/.releases/"
  cat >"$work/.releases/2026.05.08.1/rc1/release-notes.md" <<'RC_NOTES'
### @t/server
...
RC_NOTES
  ...
  cd "$work" || exit 1
  run env \
    STABLE_PACKAGE_VERSIONS="@t/server=5.1.0-rc.2,@t/web=2.3.0,not-a-version=nope" \
    ROLLUP_RELEASE_NOTES_SCRIPT="$PROJECT_ROOT/src/scripts/rollupReleaseNotes.mjs" \
    UTC_TIMESTAMP_OVERRIDE=2026-05-08T16:00:00Z \
    node "$PROJECT_ROOT/src/scripts/finalizeReleaseCycle.mjs" ".releases/2026.05.08.1"
  assert_success

  run grep -F '`@t/server@5.1.0-rc.1`' ".releases/2026.05.08.1/release-notes.md"
  assert_success
  run grep -F '`@t/web@2.3.0`' ".releases/2026.05.08.1/release-notes.md"
  assert_success
  run grep -F '`@t/server@5.1.0-rc.2`' ".releases/2026.05.08.1/release-notes.md"
  assert_failure
}
