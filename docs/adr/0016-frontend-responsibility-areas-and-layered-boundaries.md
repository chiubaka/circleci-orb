...
**Monorepo package taxonomy** ([ADR 0032](0032-monorepo-package-taxonomy-naming-and-domain-contracts.md)) and **frontend package split vs this ADR** ([ADR 0033](0033-frontend-package-split-application-of-adr-0032-and-adr-0016.md)): when the repo uses **`packages/frontend/core`** and **`packages/frontend/react`**, the **same** layer **meanings** below apply, but **physical boundaries** often follow **packages** first—**core** holds **domain / application / infrastructure** slices; **react** holds **presentation** slices; **`apps/*`** hosts **`app/`**-style composition. ADR 0033 is the explicit bridge between ADR 0032 and this ADR. **Shared** **`@<scope>/domain`** and **`@<scope>/contracts/*`** follow ADR 0032; **contracts** stay at **infrastructure** trust boundaries, not in dumb presentation.
